...
@HiltViewModel
class SiteGuideViewModel @Inject constructor(private val guideRepository: GuideRepository) : ViewModel() {

    private var _guideSites = MutableStateFlow<List<Site>>(emptyList())
    val guideSites = _guideSites.asStateFlow()

    suspend fun getGuideSitesByUserId(id: Int) {
        _guideSites.value = guideRepository.getGuideSitesByUserId(id)
    }

    init {
        viewModelScope.launch {
            getGuideSitesByUserId(1)
        }
    }
