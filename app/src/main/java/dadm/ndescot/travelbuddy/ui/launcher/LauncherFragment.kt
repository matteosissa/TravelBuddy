...
/**
 * LauncherFragment is the entry point of the application.
 * It decides whether to show the WelcomeFragment or the MainLayoutFragment based on the user's first access.
 */
@AndroidEntryPoint
            ...
            viewModel.isFirstAccess.collect { isFirstAccess ->
                ...
                if (isFirstAccess) {
                    try {
                        ...
                        Toast.makeText(
                            requireContext(),
                            "There was an error while trying to enter the application",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                } else {
